...
* Библиотека должна быть разработана на языке Swift.
...
* Добавь в Makefile цель _s21_graph_algorithms.a_.
...
* Используй *самописные* вспомогательные классы `Stack` и `Queue` (для этого можно переиспользовать своё решение из проекта *SWIFT2*). Эти классы должны содержать в себе следующие методы:
